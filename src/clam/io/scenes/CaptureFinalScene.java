...
public class CaptureFinalScene extends Scene {
	public CaptureFinalScene () throws IOException {
		...
		TitleLayer title = new TitleLayer ("(c) 2016 Christopher Sloetjes");
		ProgressBarLayer progress = new ProgressBarLayer (frameCount);
		for (int i=0;i<frameCount;i++) {
			Frame frame = new Frame ();
			frame.layers.add(new SolidBarLayer(Color.RED,0,0,1920,1080));
